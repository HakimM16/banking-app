...
// API functions for interacting with the backend endpoints
    export const api = {
        ...
        getUserProfile: async (userId: number) => {
            const response = await axios.get<User>(`${API_BASE_URL}/api/user/${userId}`);
            return response.data;
        },

        // Update user profile
        updateUserProfile: async (userId: number, data: UpdateUserFormInputs) => {
            const response = await axios.put<User>(`${API_BASE_URL}/api/user/${userId}`, data);
            return response.data;
        },

        // Create a new address for the user
        createUserAddress: async (userId: number, data: CustomiseAddressFormInputs) => {
            const response = await axios.post<Address>(`${API_BASE_URL}/api/user/${userId}/create_address`, data);
            return response.data;
        },

        // Update user's address
        updateUserAddress: async (userId: number, data: CustomiseAddressFormInputs) => {
            const response = await axios.put<Address>(`${API_BASE_URL}/api/user/${userId}/update_address`, data);
            return response.data;
        },

        // Get user's address
        getUserAddress: async (userId: number) => {
            const response = await axios.get<Address>(`${API_BASE_URL}/api/user/${userId}/address`);
            return response.data;
        },
        ...
        getAccounts: async (userId: number) => {
            const response = await axios.get<Account[]>(`${API_BASE_URL}/api/accounts/user/${userId}`);
            return response.data;
        },

        // Get count of active accounts for a user
        getActiveAccounts: async (userId: number): Promise<AccountCount> => {
            const response: AxiosResponse<AccountCount> = await axios.get<AccountCount>(`${API_BASE_URL}/api/accounts/${userId}/active-accounts`);
            return response.data;
        },

        // Get total balance for a user
        getBalance: async (userId: number) => {
            const response = await axios.get<TotalBalanceResponse>(`${API_BASE_URL}/api/accounts/${userId}/total-balance`);
            return response.data;
        },
        ...
        createAccount: async (userId: number, data: CreateAccountFormInputs) => {
            const response = await axios.post<Account>(`${API_BASE_URL}/api/accounts/${userId}`, data);
            return response.data;
        },

        // Update account status
        updateAccountStatus: async (userId: number, accountId: number, data: UpdateAccountStatusFormInputs) => {
            const response = await axios.patch<Account>(`${API_BASE_URL}/api/accounts/${userId}/${accountId}/status`, data);
            return response.data;
        },
